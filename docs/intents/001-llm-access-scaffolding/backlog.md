...
|---|---|---|---|---|
| 01 | `app llm chat "<prompt>"` answers from the configured OpenRouter chat model and prints that call's prompt/completion tokens and its USD cost | – | #1 | running |
| 02 | Every gateway failure the seam can meet surfaces as its own domain code behind one generic failure line, the provider's own wording only under `--details` | 01 | #2 | open |
...
| 06 | `app prompt show <capability>/<kind>/<id>` prints the text of a named prompt version; an unknown id and a malformed id fail differently | – | #6 | open |
