...
| 03 | Adventure progress cannot contradict itself: a campaign run records one row per adventure entered, refuses the same adventure twice, refuses a second *active* adventure beside the first, and refuses a row marked completed with no completion time | 02 | #16 | running |
