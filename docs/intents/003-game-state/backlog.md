...
| 05 | The transcript is append-only, ordered and filterable: events written to a campaign run read back in write order by id alone, a DM-visibility event is absent from the player read while still present in the table, an unknown type or visibility is refused, and per-turn and per-run cost are exact sums over the rows | 02 | #18 | running |
