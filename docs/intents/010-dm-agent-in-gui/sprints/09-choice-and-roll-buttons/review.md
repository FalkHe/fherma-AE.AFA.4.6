...
mid-way still keeps the thinking line until the long-wait sprint. In live play the Dungeon Master rolls every check itself and never hands the dice to the hero, so the roll button only appears when the game actually asks for a roll; an attempt to change that through its instructions made it ask the player which ability to use instead, and was taken out again. Teaching the Dungeon Master when to hand the dice over is proposed as its own work.
