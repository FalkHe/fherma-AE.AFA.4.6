...
which exit they take — never a destination — so nobody can be moved somewhere the story does not connect to.
An ordinary exit moves them and the transcript says where they went. An exit that ends the adventure completes
it instead, and since Greenhollow's is the campaign's only one, the game itself finishes. Nobody is swept off
the board: everyone stays where they stood, so the ending can still be described.
...
- A game can now be started, given a character, entered, played through its scenes and brought to an end.
  What is still missing before it is playable is the turn itself, and dice.
