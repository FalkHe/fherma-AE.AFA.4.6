...
|---|---|---|---|---|
| 1 | backend-python | Starting a campaign run: it pins the content version, gives its starter the owning membership, brings every object the campaign declares into being unplaced; listing a user's runs; reading one. Access is by membership, and another's run looks like one that does not exist | AC1–AC3: `setup`, version pinned; one object per placement and per carried item, none positioned, no event; unknown campaign and foreign run raise their own errors | – |
| 2 | backend-python | The three endpoints over that service, in the wire's vocabulary, every failure leaving as the one error envelope | AC1, AC2: 201 and the camelCase body; unauthenticated and CSRF-less calls refused; failures answer the envelope | WI1, I1–I5 |
| 3 | backend-python | Module README and the playthrough doc replace "there is no surface" with the endpoints and service functions | AC4 | I1–I5 |
| qa | qa | Black-box acceptance tests, one per criterion | below | I1–I5 |

## Interfaces
- I1 `api_router.include_router(playthrough_routes.router, prefix="/playthrough", tags=["playthrough"])`.
...
- I5 `instance_key` (003 ASSUMPTION 9): `<adventure>:<scene>:<template>:<ordinal>` for a placement, ordinal `1..count`; `<owner_key>/<template>:<ordinal>` for a carried one, per placement instance. `greenhollow/v1` implies thirteen. Uniqueness on `(campaign_run_id, instance_key)` holds *within one run*: its world is instantiated exactly once. It never refuses a second run of the same campaign — 003 guarantees a user may start one repeatedly. No pre-check; an instance-key `IntegrityError` becomes `ALREADY_STARTED`. Run ids are fresh ULIDs. Placements flush before carried rows; no ORM relationship orders them.
...
- AC2 → the list holds only the caller's runs, newest first, archived included; another's run and an unknown id alike not found.
- AC3 → `greenhollow/v1` writes exactly the thirteen keys, unpositioned, version pinned, no event; a second start is a second run, with no key shared.
