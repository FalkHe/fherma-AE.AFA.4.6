...
- A test from the knowledge-base work had begun failing for its own reasons: it ran the whole suite inside one
  of its own tests, on a time budget the suite had outgrown. It now checks the same thing at a small fixed cost,
  which also took the routine test run from over two minutes to about ten seconds.
