...
| 03 | `app srd ingest` embeds the stored source and the corpus goes from empty to populated: the run prints chunk count, tokens and USD cost, and `app srd status` then reports rows, model and ingest time | 01, 02 | #10 | running |
