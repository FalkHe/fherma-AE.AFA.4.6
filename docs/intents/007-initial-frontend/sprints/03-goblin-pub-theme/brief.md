...
frontend, drive the theme from them as a dark-only scheme, load the display, sans and mono faces, and use a suitable
icon package for the design's Lucide icons. Rename the product to "The Goblin's Tavern" and
...
- AC5: Lint, typecheck and tests pass, and `frontend/src/structure.test.ts` states the new pinned file list and icon
  package allowance with their reasons.
...
- The design bundle's compiled React components are **not** consumed — only its tokens, themed through component
  ...
  tokens).
- Fonts follow the design system's own loading note; icons come from a suitable Lucide-compatible package.
