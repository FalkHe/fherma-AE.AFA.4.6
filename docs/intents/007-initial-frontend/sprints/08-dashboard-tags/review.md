...
Round 2: changes requested — the tags, the filter and the archived group are right and unchanged, and the screen's
written description now matches what a player sees. One rule in it was still stated backwards: it said an archived
campaign never shows anything to click, while a campaign whose story content has gone missing does still offer its
"why can't I open this?" explanation even when archived. That sentence was corrected after the verdict; the
two-round limit means this verdict stands, so the merge request is a draft for you to release.
