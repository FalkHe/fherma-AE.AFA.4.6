...
| State | The business model of a playthrough — run, progress, object state, event, settings — and the ownership model | 3 |
| State | Its data model as real tables: the content-version pin, object instances with promoted stats, the append-only event stream, the per-run settings store | 3 |
...
| State | Run lifecycle — start, list, open, archive, unarchive, resume — and the settings write path | 5 |
| State | The validated object write path, and scene placement and advance | 5 |
| State | Event append and cost accounting — append-only, visibility-filtered, cost is a sum | 5 |
| Mechanics | Dice — expression parse, RNG, visibility flag — inside the transition that records the roll | 5 |
...
- **Game state is modelled before it is changed, and the split is along
  model/behaviour, not along domain.** Phase 3 owns the entities, their
  relations, the ownership model and the tables; phase 5 owns every path that
  writes them. Two phases of one subject rather than one, because closing the
  model's open decisions is a design job with its own evidence — a migration
  and a read — and a service written against an unsettled model gets rewritten.
- **Dice are not a service.** A dice expression resolves to a number and
  changes nothing, so it lives inside the state transition that records the
  roll, not in a layer of its own — and the DM's `roll_dice` binding remains
  its sole caller, forever.
...
Content schema & loader ──> the authored set (it validates it),
                            the state model (version pin + object instances),
...
The state model + its tables ──> every state service, journal memory,
                                 character generation (somewhere to write
                                 the sheet)
Run lifecycle ──> character generation, the playthrough browser
Validated object writes ──> scene placement and advance, the state display,
                            the update_object binding
Event append ──> cost accounting, trace pane, turn failure & recovery
Dice, inside the roll transition ──> the roll_dice binding
...
- **A tool binding is not a build dependency of the service it exposes — the
  ...
  it is what lets rule lookup, the state services with dice inside them, and
  journal memory land finished in phases 4, 5 and 6 while their bindings arrive
  together in 8.
- **Phases 1–6 and 8 are demo-dependent on a later phase for a screen; none of
  ...
  surfaces it.
- **Dice resolution, rule lookup and journal memory are demo dependencies of
  nothing.** Each is provable on its own terms; the agent makes them *useful*,
  not *verifiable*.
...
- **Phases 3 and 5 are headless by design, and phase 7 surfaces them.** A run
  no player can start is not a gap — it is what makes "no character, no game"
  true by construction, since the player-facing way to begin a run only ever
  ships alongside character creation.
- **Phase 3 is build-dependent on phase 1 and phase 5 is build-dependent on
  phase 3, and this is the reason the two exist separately.** A mechanic acts
  against state; a write path validates against a schema. Neither can be
  written, let alone tested, before the tables are pinned — which is why the
  earlier plan's parallel "Game Mechanics" phase was not buildable as drawn.
...
- **Group A = {1, 2}** — the two phases depending on nothing but phase 0.
  **They share one file: `backend/app/cli.py`**, where both register a command.
  It needs an explicit order or a single owner for the registration lines. The
  hazard is the registrations, not the commands. Phase 4 joins the same hazard
  as soon as it registers `app srd`.
- **Group B = {3, 4}** — disjoint domains: phase 3 needs phase 1, phase 4
  needs phase 2, and neither needs the other.
- **Group C = {4, 5}** — still disjoint once phase 3 has landed, which is why
  phase 4 can span both groups.
- **Group D = {6, 7}** — disjoint.
...
- **The revision chain is linear.** Phases 3 and 4 would otherwise stack
  revisions on the same `down_revision` while both adding a line to
  `alembic/env.py`, and phases 6 and 7 both alter phase 3's tables — so the
  phases are assigned an explicit revision order and each rebases onto the
  previous head. The hazard is the heads, not the migration content. **Phase 5
  is expected to add no revision at all**: it writes the tables phase 3
  created, and a migration in phase 5 means the model phase missed something.

**Longest chain of build dependencies:** 1 → 3 → 5 → 7 → 8 → 9 → 10 → 11,
eight deep. Splitting game state in two lengthened the chain by one; the same
work in one phase would have been the largest milestone in the stage after
phase 8, with its model decisions and its write paths reviewed together.

## 4. Three verdicts worth keeping

**State before the services that change it, and no mechanics layer.** Version 2
of the plan drew "Game Mechanics" as a phase parallel to content and the
provider seam, and "Runs and Game State" as one phase after content. Both were
wrong in the same way: a mechanic is a state transition, so mechanics parallel
to state is a phase that cannot be built, and a single state phase reviews its
model decisions and its write paths in one milestone. The replacement splits on
model versus behaviour — phase 3 pins the entities, the ownership model and the
tables, phase 5 owns every path that writes them — and dissolves mechanics into
phase 5, because arithmetic over a roll that gets recorded is part of recording
it and does not earn a service, a module or a milestone. What is *not* claimed
is that the split was free: it lengthened the longest build chain to eight, and
phase 3 ships with no behaviour to demonstrate beyond a migration and a read.

**Services first, bindings with the agent.** The question was whether to land
...
registry exists to receive it. So the services land in phases 4, 5 and 6, and
...
| 4 | Appropriate tools, error handling, real-world usage | Phases 2 (provider failures), 4 (empty retrieval), 5 (rejected writes, invalid dice expressions), 8 (turn failure and recovery, refusals) |
...
|---|---|---|
| **Medium-1** — token usage and cost | Summed from the event stream; per turn on the trace row, run total in the play-screen header and on the list row | Stream table at 3, accounting at 5, first real figures at 7, **display at 9** |
...
| Medium-4 — authentication and personalisation | Landed auth plus per-run settings | Phase 0 + phase 3 (the ownership model and the settings store) + phase 5 (its write path) |
...
| **How** a fixture-local real engine is driven — not whether one is allowed, which `backend-stack.md` already permits. Doing it against async SQLAlchemy collides with the landed "fully synchronous suite, no `pytest-asyncio`, no async fixtures" rule, so this **may amend a landed decision**. Phase 3 inherits the answer, as the first phase with tables of its own to read back | 4 |
| Ownership of a run: a membership row, or a user id on the run. **`model.md` and `architecture.md` currently rule for the membership row** — phase 3 may overturn that ruling, and §9 records the correction it must then make. **Closed by D10 and D13 (intent 003): upheld — ownership lives on the membership row, the `campaign_run_members` table that landed alongside `campaign_runs`** | 3 |
| The adventure's progress: its own entity, or fields on the run. **`model.md` currently rules for a separate entity** — same condition. **Closed by D10: upheld — the adventure run is its own entity, the `adventure_runs` table, one row per adventure entered** | 3 |
| Where the state models live: their own module, or `playthrough`. **Settled in 3, not deferred** — phase 5's service boundary follows the module boundary, so a service phase left to choose it would be choosing its own shape. **Closed by D14: the `playthrough` module holds all five state tables and the mutations that change them; `game` stays separate as the agent's home** | 3 |
| Whether the event stream's cost is stored per event or recomputed, given that phase 9 reads a run total and a per-turn figure from the same rows. **Closed by D4: cost is stored per event on `events`, reportable to the owner and never shown to the player** | 3 |
| The mechanic set: which state transitions the game actually needs, and which one each mechanic is. **The output is a list of transitions, not a layer** — a mechanic that changes nothing is arithmetic inside the service that records its result | 5 |
| The read surface for state: one service per entity, or one `playthrough` service that owns every transition | 5 |
...
- **Internal plumbing** — the OpenRouter seam and prompt assets (2), dice (5).
  Nobody drives these directly at all; they are reached through a capability
  that does have a surface.
- **Headless backends** — the state model (3) and the services that change it
  (5), journal memory (6), and the DM turn (8). These *are* player
  capabilities, and **this document does
...
The surfacing phases are 1 → 7 and 8 · 2 → 8 and 10 · 3 → 7 · 4 → 9 ·
5 → 7, 8 and 9 · 6 → 9 · 8 → 9. Phase 11's documentation makes the argument explicitly,
...
| The combat paragraph, the encounter node in the entity diagram, "Encounter hangs off AdventureRun", and the encounter line in the lifecycle table | `general/model.md` | 3 |
| The **Encounter** run-term entry, and the **Initiative** game term, which now has no caller | `general/glossary.md` | 3 |
| Player-experience §3's "turn order beside the narration" | `general/app-vision.md` | 3 |
| The web-client bullet's "state panel (HP, AC, inventory, turn order)" | `general/architecture.md` | 3 |
| The lifecycle table's **Purge** row describes a capability §5 fences out of Stage-01 | `general/model.md` | 3 |
| **A confirmation, not a correction.** The lifecycle table's "generate the player's creature" on starting a playthrough is **correct as written** — phase 7 does exactly that, which is why character creation and the run are adjacent rather than swapped. Only the **seed character's fixture role** needs stating: it exists so phase 3 has a character to instantiate object rows from before the generation agent lands | `general/model.md` | 3 |
| Nothing names a **seed player character**, so the fixture role above has no term | `general/glossary.md` | 3 |
| **Conditional, and it must be resolved either way.** "Ownership lives on the membership row" is written as a *ruling* in one file and repeated in the other, while §7 records it as an *open decision*. If phase 3 overturns it, phase 3 corrects both files; if phase 3 upholds it, phase 3 records that the decision is closed. **A phase-3 architect must not be left reading two documents that disagree about whether there is a choice**. **Closed: upheld, not overturned — D10 and D13 keep ownership on the membership row, now the `campaign_run_members` table, so both files stand as written** | `general/model.md`, `general/architecture.md` | 3 |
| **Conditional, same condition.** `AdventureRun` is written as a full entity with its own justification, while §7 records it as an open decision. Overturn and correct, or uphold and close. **Closed: upheld, not overturned — D10 keeps the adventure run a full entity, now the `adventure_runs` table, so the file stands as written** | `general/model.md` | 3 |
