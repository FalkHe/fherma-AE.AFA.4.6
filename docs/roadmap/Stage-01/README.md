...
| Ownership of a run: a membership row, or a user id on the run. **`model.md` and `architecture.md` currently rule for the membership row** — phase 3 may overturn that ruling, and §9 records the correction it must then make | 3 |
| The adventure's progress: its own entity, or fields on the run. **`model.md` currently rules for a separate entity** — same condition | 3 |
| Where the state models live: their own module, or `playthrough`. **Settled in 3, not deferred** — phase 5's service boundary follows the module boundary, so a service phase left to choose it would be choosing its own shape | 3 |
| Whether the event stream's cost is stored per event or recomputed, given that phase 9 reads a run total and a per-turn figure from the same rows | 3 |
