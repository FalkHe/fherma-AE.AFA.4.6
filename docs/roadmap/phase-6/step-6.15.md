...
Binding contract: `docs/roadmap/phase-6/shared-knowledge.md` (D1, D2, D6 — the
claim is never copied into typed columns here; hard rules). Detail source:
`docs/roadmap/model-naming-data-model.md` §2.4, §2.5, §6 (extraction rows) and
`docs/model-naming.md` (the pitfalls). The validators are 6.10's
...
- `backend/app/llm/prompts/spec_extraction.md`: extend the numbered rules
  ...
  - the two pitfalls, verbatim in spirit from `docs/model-naming.md`:
