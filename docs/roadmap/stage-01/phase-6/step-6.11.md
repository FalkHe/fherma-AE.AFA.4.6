...
Binding contract: `docs/roadmap/stage-01/phase-6/shared-knowledge.md` (**D5** — this
module is the only place parts become a string; `render_name` never queries;
the fallback is load-bearing) and `docs/roadmap/stage-01/phase-6/model-naming-data-model.md`
...
6.19/6.20 will consume). Naming semantics: `docs/modules/model-naming.md` ("Rendering
...
- Tests: **new** `backend/tests/services/test_naming_service.py`, table-driven
  over the doc's own rows — the `docs/modules/model-naming.md` "Examples" table
