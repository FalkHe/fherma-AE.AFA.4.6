...
`docs/general/architecture.md`). The router mounts at `/auth` (no `/api` prefix). JSON
...
- **Password**: min 8, max 128 chars; no other policy (strict password policy
  is explicitly out of scope per `docs/general/architecture.md`).
...
- **Stub rule (steps 1.2/1.4 only)**: until step 1.6, `useAuth.ts` contains a
  ...
  // docs/roadmap/stage-01/phase-1/shared-knowledge.md; do not change it.
