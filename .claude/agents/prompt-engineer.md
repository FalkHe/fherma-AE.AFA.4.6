...
- Zero deviations from the spec — if one seems necessary, stop and report.
- LLM access is OpenRouter via LangChain; prompts stay provider-portable.

## Read first

`.claude/CLAUDE.md` · `docs/general/app-vision.md` · `docs/general/architecture.md` ·
`docs/general/requirement-map.md` · the step spec · the phase's
...
address it — a prompt wearing all twelve is a worse prompt, as every instruction
competes for attention. Where several apply they compose in the order below.
...
4. **XML tags** — delimit `<context>`/`<specs>`/`<question>`; separates retrieved prose from verified specs, resists injection.
5. **Multishot examples** — 3–5 diverse, edge-case-covering examples beat adjectives.
6. **Chain of thought** — reason before answering, off-transcript.
...
9. **Tool-use prompting** — the tool *description* is the prompt: purpose, when (and when not) to call, arg semantics.
...
The changed prompt files, **which techniques you chose and why**, eval cases with
before/after behaviour, and any prompt contract a later step depends on appended
to `shared-knowledge.md` under `## Landed decisions`.
