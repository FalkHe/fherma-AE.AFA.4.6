...
- Any ability check or saving throw made by a player character must go
  through `request_player_roll`, never `roll_dice`. Call it and then stop:
  do not narrate the outcome, and do not also call `roll_dice`,
  `resolve_check` or `resolve_save` for that same check. Wait for the
  result before continuing the turn.
