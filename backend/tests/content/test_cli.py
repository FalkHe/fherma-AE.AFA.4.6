...
`configure_logging()` never runs on that path); criterion 51 must run
...
def test_valid_tree_exits_0_stdout_ok_stderr_empty_c44(content_root):
...
def test_one_broken_version_exits_1_and_reports_every_error_entry_c45(content_root):
...
def test_no_campaigns_directory_c47(content_root):
    result = runner.invoke(content_app, [])
...
def test_empty_campaigns_directory_c47(content_root):
...
def test_nonconformant_version_directory_reported_alongside_valid_one_c48(content_root):
    build_version_dir(content_root, version="v1")
...
def test_campaign_with_no_conformant_version_directory_c49(content_root):
...
def test_cli_reads_content_root_at_call_time_not_import_time_c50(content_root):
    build_version_dir(content_root)
...
def test_no_log_record_on_stdout_or_stderr_via_the_real_cli_callback_c51(content_root):
    build_version_dir(content_root, campaign=campaign(id="wrong-id"))
...
def test_help_lists_content_group_and_validate_command_c52():
