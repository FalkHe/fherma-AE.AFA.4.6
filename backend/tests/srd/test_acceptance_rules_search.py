...
own `cosine_distance` then does with them is exercised for real. The
research note behind this sprint's plan measured exactly this recipe
(`e0`, `e1`, `e2` rows against a `0.9*e0 + 0.4*e1` query) returning
distances `0.086 / 0.594 / 1.0` against the real, migrated `srd_rules`
table -- `test_ac1`/`test_ac2` reuse it.
...
def test_ac1_search_prints_passages_most_relevant_first(srd_db, monkeypatch):
    ...
    # passages, best match first. Ranking is proved with the research
    # note's own measured recipe: rows on basis vectors e0/e1/e2, a query
    # blended mostly toward e0 -- real cosine distance against the real,
    # migrated table then orders them 0.086 / 0.594 / 1.0.
    ...
    blended_query_vector = [0.9, 0.4] + [0.0] * (VECTOR_WIDTH - 2)
